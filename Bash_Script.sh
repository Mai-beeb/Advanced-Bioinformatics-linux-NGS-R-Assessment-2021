...
# The reads required are taken as command line arguments $1 (the first argument) and $2 (the second argument), which are required for the script to run!
# The variable $# reports the number of command line arguments passed to the shell script. 
# The command line arguments to check: there are 2 arguments. 
If [ "$#" -ne 2 ]; then:
    echo "we have 2 command line arguments, one for each of the fastq files"
fi
...
# The reads required are taken as command line arguments 1 and 2 which are required for the script to run
...
echo "Running post_alignment_filtering"
...
echo "Running Variant Annotation using Annovar"
...
echo "Running Variant Annotation using snpEFF"
